...
// Función para convertir "APELLIDO Nombre" a "Nombre Apellido"
...
// Ranking Anual 2025 - Solo torneos SIN handicap
...
async function importRankingAnual2025() {
    try {
        console.log('📊 IMPORTANDO RANKING ANUAL 2025\n');
        console.log(`   Total jugadores: ${rankingAnual2025.length}\n`);
        ...
        for (const entry of rankingAnual2025) {
            const { firstName, lastName } = parsePlayerName(entry.name);
            ...
            if ((updated + created) % 20 === 0) {
                console.log(`   Progreso: ${updated + created}/${rankingAnual2025.length}`);
            }
        }
        ...
        console.log(`   Rankings creados:      ${created}`);
        console.log(`   Rankings actualizados: ${updated}`);
        ...
        const totalRankings = await prisma.ranking.count({
            where: { discipline: 'THREE_BAND_ANNUAL' }
        });

        console.log(`✅ Total Rankings Anuales en BD: ${totalRankings}\n`);
