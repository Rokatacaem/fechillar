                            ...
                            <td className="p-4 font-bold text-white flex items-center gap-3">
                                {rk.player.photoUrl ? (
                                    ...
                                    <div className="w-10 h-10 rounded-full shrink-0 flex items-center justify-center font-bold text-xs" style={{ backgroundColor: secondaryColor, color: '#fff' }}>
                                        {([rk.player.firstName, rk.player.lastName].filter(Boolean).join(' ') || rk.player.user?.name || "??").substring(0,2).toUpperCase()}
                                    </div>
                                )}
                                <span className="group-hover:text-emerald-400 transition-colors">
                                    {[rk.player.firstName, rk.player.lastName].filter(Boolean).join(' ') || rk.player.user?.name || "Jugador Provisional"}
                                </span>
                            </td>
